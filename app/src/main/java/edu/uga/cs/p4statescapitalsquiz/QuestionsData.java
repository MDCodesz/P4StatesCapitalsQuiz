...
public class QuestionsData {
    ...
    private static final String[] allColumns = {
            StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_ID,
            StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_STATE,
            StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CAPITAL,
            StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CITYONE,
            StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CITYTWO
    };
    ...
    public List<Question> retrieveAllQuestions() {
        ...
        try {
            ...
            cursor = db.query( StateCapitalsQuizDBHelper.TABLE_QUESTIONS, allColumns,
            ...
            if( cursor != null && cursor.getCount() > 0 ) {
                ...
                while( cursor.moveToNext() ) {
                    ...
                    if( cursor.getColumnCount() >= 5) {
                        ...
                        columnIndex = cursor.getColumnIndex( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_ID );
                        long id = cursor.getLong( columnIndex );
                        columnIndex = cursor.getColumnIndex( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_STATE );
                        String state = cursor.getString( columnIndex );
                        columnIndex = cursor.getColumnIndex( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CAPITAL );
                        String capital = cursor.getString( columnIndex );
                        columnIndex = cursor.getColumnIndex( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CITYONE );
                        String cityOne = cursor.getString( columnIndex );
                        columnIndex = cursor.getColumnIndex( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CITYTWO );
    ...
    public Question storeQuestion( Question quizQuestion ) {
        ...
        values.put( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_STATE, quizQuestion.getStateName());
        values.put( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CAPITAL, quizQuestion.getCapital() );
        values.put( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CITYONE, quizQuestion.getCityOne() );
        values.put( StateCapitalsQuizDBHelper.QUESTIONS_COLUMN_CITYTWO, quizQuestion.getCityTwo() );

        // Insert the new row into the database table;
        // The id (primary key) is automatically generated by the database system
        // and returned as from the insert method call.
        long id = db.insert( StateCapitalsQuizDBHelper.TABLE_QUESTIONS, null, values );

        // store the id (the primary key) in the Question instance, as it is now persistent
        quizQuestion.setId( id );

        Log.d( DEBUG_TAG, "Stored new job lead with id: " + String.valueOf( quizQuestion.getId() ) );
